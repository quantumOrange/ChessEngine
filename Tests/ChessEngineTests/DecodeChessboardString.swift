...
class DecodeChessboardString: XCTestCase {
    ...
    func testStringToBoardAndBack() {
        ...
        {
            return string.filter { !$0.isNewline && !$0.isWhitespace  && !$0.isNumber && !$0.isLetter }
        }
    ...
    func testStringToBoardAndBackWithRankAndFileAnnotations() {
    ...
    let chessString = """
        ...
        XCTAssertEqual(pruned(chessString), pruned(newString), "\(pruned(chessString)) != \(pruned(newString))")
